...
If the header is present the control goes to the request handler. Perform the user and product validation. Check if the user exists as well as whether the product exists. Return a0n error with a suitable error message if either of these validations fail
